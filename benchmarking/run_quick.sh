...
if [ ! -f $out_file ]; then
    echo "Executable, n, CPU Time (ns), Iterations" > $out_file
fi
...
for executable in "$@"; do
	...
	for file in $(dirname "$0")/../permutations/*; do
		...
		cpu_time_ns=0
		new_iterations=1
		while [ $cpu_time_ns -le 100000000 ]; do # 0.1 second
			iterations=$new_iterations
			cpu_time_ns=$(get_cpu_time)
			echo "At $iterations iterations took $cpu_time_ns (ns)"
			new_iterations=$(($iterations * 2))
		done
		echo $(basename "$executable" .sh),$(basename "$file" _pair.txt), $cpu_time_ns, $iterations >> $out_file
	done
done
